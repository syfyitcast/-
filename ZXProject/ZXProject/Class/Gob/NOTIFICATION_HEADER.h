//
//  NOTIFICATION_HEADER.h
//  ZXProject
//
//  Created by 刘清 on 2018/3/22.
//  Copyright © 2018年 com.nexpaq. All rights reserved.
//

#ifndef NOTIFICATION_HEADER_h
#define NOTIFICATION_HEADER_h


#pragma mark --------------------------------- NOTIFICATION_KETY

#define NOTIFI_READCOUNT @"NOTIFI_READCOUNT"
#define NOTIFI_PROJECTLISTDONE @"NOTIFI_PROJECTLISTDON"
#define NOTIFI_WORKFLOWRELOADDATA @"NOTIFI_WORKFLOWRELOADDATA"
#define NOTIFI_WORKTASKRELOADDATA @"NOTIFI_WORKTASKRELOADDATA"
#define NOTIFI_WORKEVENTRELOADDATA @"NOTIFI_WORKEVENTRELOADDATA"
#define NOTIFI_GETLOCATIONINFO @"NOTIFI_GETLOCATIONINFO"
#define NOTIFI_DEVICEINFORELOADDATA @"NOTIFI_DEVICEINFORELOADDATA"



#pragma mark --------------------------------  字典的key

#define BUSITRAVEL_TRANSMODE  @"BUSITRAVEL_TRANSMODE"
#define FEE_TYPE              @"FEE_TYPE"
#define REPORT_TYPE           @"REPORT_TYPE"




#endif /* NOTIFICATION_HEADER_h */


