...
- (void)clickSendCodeBtn{
    ...
    if(isMatch) { //有效手机号
        [self getCode];
        [HttpClient zx_httpClientToGetVerifyCodeWithType:1 andMobile:self.mobileField.text andSuccessBlock:^(int code, id  _Nullable data, NSString * _Nullable message, NSError * _Nullable error) {
            if (code == 0) {
...
- (void)clickRegisterAction{
    ...
    if(isMatch) { //有效手机号
        if (self.screCodeField.text.length == 6) {
            if (![NetworkConfig sharedNetworkingConfig].smsid) {
                ...
                [HttpClient zx_httpClientToRegisterWithMobile:self.mobileField.text andSmcontent:self.screCodeField.text andSmsid:[NetworkConfig sharedNetworkingConfig].smsid andPassword:self.pwdTextField.text andSuccessBlock:^(int code, id  _Nullable data, NSString * _Nullable message, NSError * _Nullable error) {
                    ...
                    if (code == 0) {
                        ...
                        if ([message isKindOfClass:[NSNull class]] || message.length == 0) {
                            [MBProgressHUD showError:[NSString stringWithFormat:@"注册失败 code = %zd",code]];
                        }else{
                            [MBProgressHUD showError:message];
                        }
                    }
...
- (FButton *)submitBtn{
    if (_submitBtn == nil) {
        ...
        [_submitBtn addTarget:self action:@selector(clickRegisterAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _submitBtn;
}
