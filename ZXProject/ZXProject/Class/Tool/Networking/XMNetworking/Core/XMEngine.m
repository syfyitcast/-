...
- (void)xm_dataTaskWithRequest:(XMRequest *)request
    ...
    dataTask = [sessionManager dataTaskWithRequest:urlRequest
        ...
        if (urlresponse.statusCode == 401) {
            ...
            BOOL isSendLogoutMessage = [userDefault boolForKey:@"isSendLogoutMessage"];
            if (!isSendLogoutMessage) {
                [userDefault setBool:YES forKey:@"isSendLogoutMessage"];
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName: @"LOGINATOTHERDEVICE" object:nil];
                });
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [userDefault setBool:NO forKey:@"isSendLogoutMessage"];
